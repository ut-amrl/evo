...
def parser() -> argparse.ArgumentParser:
    ...
    coda_parser = sub_parsers.add_parser(
        "coda", parents=[shared_parser],
        description="{} for CODa pose files - {}".format(basic_desc, lic))
    coda_parser.add_argument("ref_file",
                              help="reference pose file (ground truth)")
    coda_parser.add_argument("est_file", help="estimated pose file")
    ...
    for trajectory_parser in {
            bag_parser, bag2_parser, euroc_parser, tum_parser, coda_parser
    }:
